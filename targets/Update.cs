...
public static partial class Update
{
    ...
    public static async Task Upstream()
    {
        ...
        var xUnitVersion = Version.Parse(xUnitVersionRange[1..xUnitVersionRange.IndexOf(',')]).ToString();
        using var httpClient = new HttpClient();
        foreach (var upstreamSource in Directory.EnumerateFiles(Path.Combine(srcDir, "LambdaTale.Execution", "Upstream"), "*.cs"))
        {
            var header = File.ReadLines(upstreamSource).First();
            ...
            await using var newContent = await httpClient.GetStreamAsync(newUpstream);
